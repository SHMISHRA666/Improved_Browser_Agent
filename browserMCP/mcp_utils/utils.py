...
async def ensure_browser_session():
    ...
    # Check if browser session exists and is still valid
    if browser_session is None or not browser_session.initialized:
        # If browser session exists but is not initialized, clean it up
    ...
    # Additional check to ensure browser context is still valid
    try:
        if browser_session.browser_context and browser_session.browser_context.pages:
            # Try to access the browser context to see if it's still valid
            await browser_session.browser_context.pages[0].evaluate('1')
        else:
            # Browser context is invalid, reinitialize
            await browser_session.stop()
            browser_session = None
            controller = None
        ...
        # Browser context is invalid, reinitialize
        print(f"Browser session became invalid, reinitializing: {e}")
        try:
            await browser_session.stop()
        except:
            pass
...
async def get_browser_session():
    ...
    # Additional validation to ensure the session is still working
    try:
        if browser_session and browser_session.browser_context:
            # Test if we can access the browser context
            pages = browser_session.browser_context.pages
            if pages:
                await pages[0].evaluate('1')
        else:
            # Session is invalid, reinitialize
            await ensure_browser_session()
    except Exception as e:
        print(f"Browser session validation failed, reinitializing: {e}")
        await ensure_browser_session()
